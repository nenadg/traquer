...
Controls.prototype.timeline=function(){var a=this.traquer.recordedEvents,b=document.querySelector("ol.timeline-container");b||(b=document.createElement("ol"),b.className="timeline-container",document.body.appendChild(b));b.innerHTML="";var c=b.getBoundingClientRect().width,e=a[a.length-1].time;this.tlcWidth=c;var d=2147E6,f=[],h="",l,m;for(m in a)if(a.hasOwnProperty(m)){var k=a[m],g=parseInt(100*(k.time/e)),g=parseInt(g*c/100),p="e_"+Math.random().toString(36).substring(3).substr(0,8),n=['<li id="'+
p+'"',' style="left: '+g+'px;"',' class="{css-class}">','<div class="tl-type" style="z-index: '+d++ +'">',"{title} {key} </div>",'<div class="tl-selector" style="z-index: '+d++ +'">',"{selector}</div></li>"].join(""),q="click"==k.type||"keydown"==k.type||"keyup"==k.type||"keypress"==k.type?"red":-1<k.type.indexOf("DOM")?"blue":-1<k.type.indexOf("select")?"green":"default",n=n.replace(/{title}/igm,k.type).replace(/{selector}/igm,k.selector).replace(/{key}/igm,k.raw.key||"").replace(/{css-class}/igm,
q),k={id:p,time:k.time,type:k.type,lastTimeLine:g};f.push(k);h+=n}b.innerHTML=h;for(m in f)f.hasOwnProperty(m)&&(k=f[m],a=k.type,d=k.id,g=parseInt(100*(k.time/e)),g=parseInt(g*c/100),l&&(g>l-10&&g<l+10)&&(l=b.querySelector("#"+d),d=l.previousSibling,g=window.getComputedStyle(l),d=window.getComputedStyle(d),g=Math.abs(Math.round(100*parseFloat(g.top.replace("px",""))/100)),d=Math.abs(Math.round(100*parseFloat(d.top.replace("px",""))/100)),h=0,"click"==a||"keydown"==a||"keyup"==a||"keypress"==a||-1<
a.indexOf("DOM")||-1<a.indexOf("select")?(d==g&&(h=g+8),d>g&&(h=d-g+4),d<g&&(h=g-d+4),h=-h):(d==g&&(h=g+18),d>g&&(h=d-g+38),d<g&&(h=g-d+38),h-=70),h+="px",l.style.cssText+="top: "+h+";"),l=k.lastTimeLine)};
...
(d.className+=" playing",this.play(a))}.bind(b,c)));e||(e=document.createElement("div"),e.className="timeline-track",document.body.appendChild(e),e.innerHTML='<div class="current-event"></div>',b.trackTarget=e,e.addEventListener("move",function(b){var c=b.detail.eventInfo,e=b.detail.last,d=c.time,k=a.tlcWidth,g=document.querySelector(".current-event"),e=parseInt(100*(d/e)),k=parseInt(e*k/100)+152;b.target.style.cssText="left: "+k+"px;";g.innerHTML=a.previousTime>d-2&&a.previousTime<d+2?g.innerHTML+
(c.type+"("+d+") <br />"):c.type+"("+d+") <br />";a.previousTime=d}))};"use strict";var EventBase=function(a){if(!(this instanceof EventBase))return new EventBase;if(!(a&&a instanceof Traquer))throw Error("Can't instantiate EventBase without Traquer instance!");this.traquer=a};
...
return b},getSelector:function(a,b,c,e){var d="";if(0<a.length)for(var f in a)if(a.hasOwnProperty(f))if("value"===a[f])d+="["+a[f]+'="'+c+'"]';else if(void 0!==a[f]){var h=a[f].match(/class/g)||a[f].match(/selected/g)||a[f].match(/over/g)||a[f].match(/dirty/g)||a[f].match(/selectable/g)||a[f].match(/href/g)||a[f].match(/valign/g)||a[f].match(/role/g)||a[f].match(/tabindex/g);h||(d+="["+a[f]+"]")}if(0<b.length)for(f in b)b.hasOwnProperty(f)&&((h=b[f].match(/selected/g)||b[f].match(/over/g)||b[f].match(/dirty/g)||
b[f].match(/selectable/g)||b[f].match(/focused/g)||b[f].match(/focus/g))||(d+="."+b[f]));e&&(d+="#"+e);return d},createSelector:function(a){var b=this.targetType+a.getSelector(this.attrs,this.classList,this.value,this.id),c=document.querySelector(b),e=document.querySelectorAll(b),d;if(1<e.length)for(var f in e)if(e.hasOwnProperty(f)){var h=e[f];h.getBoundingClientRect();a.getIntersection(h,{height:10,left:a.mousePosition.x,top:a.mousePosition.y,width:10})&&(d=h==c?h.parentElement:h)}d&&d!=c&&(c=a.getAttributes(d),
e=a.getClassess(d),c.length&&e.length&&(b=a.getSelector(c,e,d.value,d.id)+" > "+b));return b},getIntersection:function(a,b){var c=a.getBoundingClientRect(),e=b.getBoundingClientRect?b.getBoundingClientRect():b,d=!1;c.left<=e.left+e.width&&e.left<=c.left+c.width&&(c.top<=e.top+e.height&&e.top<=c.top+c.height)&&(d=!0);return d},getOffset:function(a){a=a.getBoundingClientRect();return{x:a.left,y:a.top,w:a.width,h:a.height}},pointRectangleIntersection:function(a,b){return a.x>b.x1&&a.x<b.x2&&a.y>b.y1&&
a.y<b.y2},matchChild:function(a,b){var c;if(a&&a.childNodes)for(c in a.childNodes)if(a.childNodes.hasOwnProperty(c)){var e=a.childNodes[c];if(e==b)return!0;this.matchChild(e,b)}return!1},getSimilarity:function(a){var b=0,c=0,e;for(e in a)a.hasOwnProperty(e)&&(c=document.querySelector(a[e].selector))&&"hidden"!=window.getComputedStyle(c).visibility&&b++;return c=100*(b/a.length)}};
